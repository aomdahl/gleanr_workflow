...
UpdateTrackingParams <- function(sto.obj, X,W,U,V,option, sparsity.thresh = 1e-5)
{
  if(is.null(sto.obj))
  {
    sto.obj <-  list("V" = V, "U" = U, "initK" = option$K, "K" = ncol(U), "obj" = c(NA), "obj_change" = c(),
                     ...
                     "V_change" = c(), "U_change" = c(), "decomp_obj" ="")
  }
...
ConvergenceConditionsMet <- function(iter,X,W, U,V,tracker,option, initV = FALSE)
{
  ...
  #message(paste0("Objective change percent: ", obj.change.percent))

  #message(paste0("Objective change: ", objective_change))
  #message("Threshold:")
  #message(as.numeric(option[['conv0']]))
  #message("length check ", length(tracker$obj))
  #message("IS it done? ", as.numeric(obj.change.percent) <= as.numeric(option[['conv0']]) )
  #updateLog(paste0("Objective change percent: ", obj.change.percent), option)
 ...
 #If we have completed at least 1 iteration and we go up, end it.
  ...
  if(objective_change < 0 & length(tracker$obj) > 2)
  {
...
Update_FL <- function(X, W, option, preV = NULL, preU = NULL, burn.in = FALSE){
  ...
  for (iii in seq(1, option[['iter']])){ #start the iterations (although technically, we have done 1 already.)
    ...
    if(ConvergenceConditionsMet(iii,X,W,U, V, tracking.data, option, initV = is.null(preV)))
    {
      message("Convergence criteria met...")

      #Check out low pve one..
      drops <- CheckLowPVE(X,W,V) #redundant with other code
      if(length(drops) > 0 & FALSE) #we no longer do this, will need to clean up at some point
      {
