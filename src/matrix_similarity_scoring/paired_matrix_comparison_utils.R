...
pairedMatrixAlignmentByCorrelation <- function(trueL, trueF, lp, fp){
  ...
  if(all(fp == 0) & all(lp == 0))
  {
    ...
    #Jan 17- updated to return matrix of the corresponding dimension to true
    #return(list("U_r2"=0, "V_r2"=0, "reordered_U"=lp,"reordered_V"= fp, "new_order"=1:ncol(trueL), "signs"=rep(1,ncol(trueL))))
    return(list("U_r2"=0, "V_r2"=0,
                "reordered_U"=matrix(0, nrow(trueL), ncol(trueL)),"reordered_V"= matrix(0, nrow(trueF), ncol(trueF)),
                "new_order"=1:ncol(trueL), "signs"=rep(1,ncol(trueL))))
  }
