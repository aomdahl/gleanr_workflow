...
#Rscript $SRC/projectSumStats.R --output $ID/projected_hapmap3_loadings.txt --sumstats $VARIANTS/full_hapmap3_snps.z.tsv --id_type RSID --factors $FACTORS

#Make the GWAS files
echo "Making the gwas files..."
mkdir -p $ID/loading_ss_files_F
Rscript /scratch16/abattle4/ashton/snp_networks/custom_l1_factorization/src/buildSumStats.R --projected_loadings $ID/projected_hapmap3_loadings.txt --samp_file $VARIANTS/full_hapmap3_snps.n.tsv --hapmap_list $hapmap_list --output $ID/loading_ss_files_F/ --factors $FACTORS
