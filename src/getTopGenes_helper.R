...
pacman::p_load(tidyverse, optparse, data.table, magrittr)
...
loadSNPtoGeneMap <- function(path)
{
  ...
  #idiosyncrasy of the data- many genes get duplicate lines
  ...
  if(grepl(pattern="ENSG", x=snp_gene_map$gene_id[1]))
  {
    ...
    geneIDs1 <- ensembldb::select(EnsDb.Hsapiens.v79::EnsDb.Hsapiens.v79, keys= conversion.genes, keytype = "GENEID", columns = c("SYMBOL","GENEID")) %>% 
      dplyr::rename("gene_id"=GENEID)
    snp_gene_map<- left_join(snp_gene_map, geneIDs1, by= "gene_id") %>% dplyr::rename("ens_gene_id"=gene_id) %>% dplyr::rename("gene_id"=SYMBOL)
  }
  #Now pick the highest scoring one per SNP, by gene_id.
  #Note that not every gene gets a match, and that is okay- intention here is to be as inclusive as possible, just so that each SNP gets a gene assigned if possible.
  snp_gene_map %>% group_by(hg38) %>% slice_max(order_by=overall_score) %>% ungroup()
}
