                                ...
                                unique_only=FALSE)
{
  ...
  ret.list$enrichments_unique <- fread(paste0(dir, "/gene_factor_enrichments_unique/F", old.mapping, ".joined_enrichments.txt")) %>%
    set_colnames(c("Rank", "Term name", "P-value", "Odds ratio", "Combined score", "Adjusted p-value", "Genes", "gene_library"))
  ret.list
...
#sets_to_test <- list(early_Megakaryopoiesis,c(late_mk,mk_hmt), c(protoplatelet,pp_hmt),platelet_function)
