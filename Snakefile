...
rule extract_snps: #finds the variants that meet our signal threshold. here just looking at the LDSC ones.
    ...
    output:
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}.union.txt"
    shell:
        """
...
rule filter_1KG: #filter those lists for multi-allelic snps, indels, ambiguous snps, etc.
    ...
    output:
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}.ids.txt",
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}.1000G.txt"
...
rule prune: #reduce it to a pruned list
    input:
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}.1000G.txt"
    output:
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/500kb.0.04r2.prune.in"
    
    params: "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/500kb.0.04r2"
...
rule hapmap_extract: #Pull the hapmap3 snps from the LDSC summary stats. This step takes a bit more memory, 10 GB at least.
    input:
        ...
        "trait_selections/seed{seedn}_thresh{thresh}_h2-{h2}.studies.tsv"
    output:
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/full_hapmap3_snps.z.tsv",
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/full_hapmap3_snps.n.tsv"
    params:
        "/work-zfs/abattle4/lab_data/UKBB/GWAS_Neale/highly_heritable_traits_2/ldsr_format/unzipped/",
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/full_hapmap3_snps"
    shell: 
        """
        ml python/3.7-anaconda;
...
rule factorize:
    input:
        "gwas_extracts/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}.z.tsv", 
        "trait_selections/seed{seedn}_thresh{thresh}_h2-{h2}.names.tsv"
    output:
        expand("results/seed{{seedn}}_thresh{{thresh}}_h2-{{h2}}_vars{{pval}}/factorization/{ot}.{{f_type}}.{{seedn}}{ext}", ot=output_type, ext=[".txt", ".png"])
    ...
    params:
        "results/seed{seedn}_thresh{thresh}_h2-{h2}_vars{pval}/factorization/"
    run:
