...
option_list <- list(
  ...
  make_option(c("-c", "--commands"),  type = "character", default="",
              help = "Print commands to run simulations", metavar = "DIR"),
...
for (i in 1:nrow(param.setting)) {
  ...
  if (opt$commands != "") {
    cat("bash src/runSimulation.sh ", paste0(opt$output_path, '/',param.setting[i,]$fnames), paste0(opt$commands, gsub(x=param.setting[i,]$fnames, replacement = "", pattern = ".yml"),"/"), "\n")
